    ...
    <article className="pb-16">
      ...
      <section className="relative border-b border-border/60">
        ...
        <div className="container-onyx -mt-32 md:-mt-44 relative pb-8">
          ...
          <h1 className="mt-4 font-display text-4xl md:text-6xl font-bold leading-[1.05] max-w-4xl">
            {s.name}
          </h1>
        ...
        <div className="container-onyx py-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          {s.stats.map((stat) => (
            <div key={stat.label} className="text-center md:text-left">
              <p className="font-display text-2xl md:text-3xl font-bold text-electric">
                {stat.value}
              </p>
        ...
        <div className="lg:col-span-2 max-w-2xl">
          ...
          {s.pullQuote && (
            <figure className="my-10 rounded-2xl border border-electric/40 bg-gradient-to-br from-electric/10 to-transparent p-6 md:p-8">
              <svg
                className="h-8 w-8 text-electric/80 mb-3"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d="M6 17h3l2-4V7H5v6h3zm8 0h3l2-4V7h-6v6h3z" />
              ...
              {s.pullQuote.cite && (
                <figcaption className="mt-4 text-xs uppercase tracking-[0.25em] text-electric font-bold">
                  — {s.pullQuote.cite}
                </figcaption>
              )}
            </figure>
            ...
            <ol className="space-y-3">
              {s.howToUse.map((step, i) => (
                <li key={step} className="surface-card rounded-xl p-4 flex gap-4 items-start">
                  <span className="flex-none h-8 w-8 rounded-full bg-electric text-onyx-50 font-bold text-sm grid place-items-center font-display">
                    {i + 1}
                  </span>
                  <span className="pt-1 text-[15px] leading-relaxed text-foreground/90">
                    {step}
                  </span>
                </li>
              ))}
          ...
          <section className="mb-12 rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6 md:p-7">
            ...
            <ul className="mt-5 space-y-3">
              {s.important.map((i) => (
                <li
                  key={i}
                  className="flex gap-3 items-start text-[15px] leading-relaxed text-foreground/90"
                >
              ...
              {s.faqs.map((f) => (
                <details key={f.q} className="group p-5 open:bg-onyx-100/40">
                  <summary className="flex justify-between items-center cursor-pointer font-semibold text-[15px] list-none">
                    <span>{f.q}</span>
                    <span className="text-electric text-2xl transition-transform group-open:rotate-45 leading-none">
                      +
                    </span>
                  </summary>
        ...
        <aside className="lg:sticky lg:top-24 h-fit space-y-6">
            ...
            <ul className="mt-4 space-y-2.5">
              {s.benefits.map((b) => (
                <li key={b} className="flex gap-2.5 items-start text-sm text-foreground/85">
                  <span className="mt-0.5 h-4 w-4 shrink-0 rounded-full bg-electric/15 text-electric flex items-center justify-center text-[10px] font-bold">
                    ✓
                  </span>
                  <span>{b}</span>
                </li>
          ...
          <div className="surface-card rounded-2xl p-5">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Supplements support the work — they don't replace it. Pick a program that earns them.
            </p>
            <Link
              to="/programs"
