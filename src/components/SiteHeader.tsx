...
export function SiteHeader() {
  ...
  useEffect(() => {
    const routes: Array<string> = [
      "/", "/exercises", "/recipes", "/programs", "/meal-plans",
      "/challenges", "/yoga-mobility", "/builder", "/nutrition",
    ];
  ...
  const navLinks: Array<{ to: string; label: string; exact?: boolean }> = [
    ...
    { to: user ? "/my-nutrition" : "/nutrition", label: user ? t("nav.myNutrition") : t("nav.nutrition") },
  ...
  return (
    ...
    <header>
      <div className="border-b border-border bg-onyx-100/95 backdrop-blur-xl shadow-[0_1px_0_0_oklch(1_0_0_/_6%)_inset]">
        <div className={`container-onyx grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 md:grid-cols-[auto_1fr_auto] md:gap-3 ${compactAppHeader ? "h-14" : "h-16"}`}>
          <Link to="/" className="flex min-w-0 items-center gap-2.5 group">
            <div className={`${compactAppHeader ? "h-8 w-8" : "h-9 w-9"} grid shrink-0 place-items-center overflow-hidden rounded-full bg-onyx-100 ring-2 ring-electric shadow-electric`}>
              <img src={onyxLogo} alt="Onyx Elevate" className={`${compactAppHeader ? "h-5 w-5" : "h-6 w-6"} object-contain`} />
            </div>
            <span className={`truncate font-display font-bold tracking-tight ${compactAppHeader ? "text-sm" : "text-base md:text-lg"}`}>
          ...
          <nav className="hidden md:flex items-center justify-center gap-6 text-sm text-muted-foreground">
            {navLinks.map((l) => (
              <Link key={l.to} to={l.to} onClick={resetScrollablePage} activeProps={{ className: "text-foreground" }} activeOptions={l.exact ? { exact: true } : undefined} className="hover:text-foreground transition-colors">
                {l.label}
              </Link>
            ))}
          </nav>
    ...
    <div className="md:hidden border-b border-border bg-onyx-100/95 backdrop-blur-xl shadow-[0_1px_0_0_oklch(1_0_0_/_6%)_inset]">
      ...
      >
        {navLinks
          .filter((l) => !["/", "/exercises", "/my-nutrition", "/nutrition", "/my-library", "/builder"].includes(l.to))
          .map((l) => (
          <Link
            key={l.to}
            to={l.to}
