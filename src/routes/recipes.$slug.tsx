...
export const Route = createFileRoute("/recipes/$slug")({
  ...
  notFoundComponent: () => (
    <div className="container-onyx py-24 text-center">
      ...
      <Link to="/recipes" className="mt-4 inline-block text-electric">Back to recipes</Link>
    </div>
  ),
  errorComponent: ({ reset }) => (
    <div className="container-onyx py-24 text-center">
      <h1 className="font-display text-3xl font-bold">Something went wrong</h1>
      <button onClick={reset} className="mt-4 text-electric">Try again</button>
    </div>
  ),
    ...
    <div className="min-h-[100dvh] bg-background pb-12 md:pb-14">
      ...
      <div className="hidden md:block">
        ...
        <nav className="text-xs text-muted-foreground flex items-center gap-2">
          ...
          <span>/</span>
          <Link to="/recipes" className="hover:text-foreground">{t("nav.recipes")}</Link>
          <span>/</span>
      ...
      <section className="grid md:mt-6 md:grid-cols-[1.05fr_1fr] gap-4 md:gap-8 items-stretch">
        <div
          ...
          style={{ backgroundImage: `linear-gradient(to top, var(--onyx-50), transparent 40%), url(${r.image})`, backgroundSize: "cover", backgroundPosition: "center" }}
        >
          <img
            src={r.image}
            alt={r.title}
          ...
          <span className="absolute top-3 left-3 text-[10px] uppercase tracking-[0.18em] font-bold px-2.5 py-1 rounded-md bg-electric text-onyx-50 shadow-lg">{r.category}</span>
        </div>
        ...
        <div className="surface-card rounded-2xl p-5 md:p-8 flex flex-col">
          <div className="flex flex-wrap gap-1.5">
            {r.diets.map((d) => (
              <span key={d} className="text-[10px] uppercase tracking-wider font-semibold px-2 py-1 rounded bg-onyx-100 border border-border text-muted-foreground">{d}</span>
            ))}
          </div>
          <h1 className="mt-4 font-display text-2xl md:text-5xl font-bold leading-[1.1]">{r.title}</h1>
          ...
          <p className="mt-4 text-[11px] uppercase tracking-wider text-muted-foreground">{t("recipe.perServing")}</p>
      ...
      <div className="container-onyx mt-12 md:mt-16 grid md:grid-cols-[220px_minmax(0,1fr)] gap-8 md:gap-12">
        <aside className="hidden md:block">
          <div className="sticky top-24 surface-card rounded-xl p-5">
            <p className="text-[10px] uppercase tracking-[0.2em] text-electric font-semibold mb-3">{t("recipe.onThisPage")}</p>
        ...
        <div className="min-w-0 space-y-14">
          ...
          <Section id="ingredients" eyebrow={t("recipe.ingredients")} title={`${t("recipe.ingredients")} · ${r.servings} ${t("recipe.servings").toLowerCase()}`}>
            <div className="space-y-5">
              {r.ingredients.map((g, i) => (
                <div key={i} className="surface-card rounded-xl p-5">
                  {g.title && <p className="text-xs uppercase tracking-wider text-electric font-semibold mb-3">{g.title}</p>}
                  <ul className="space-y-2 text-sm">
                    {g.items.map((it) => (
          ...
          <Section id="instructions" eyebrow={t("recipe.instructions")} title={t("recipe.stepByStep")}>
            <ol className="space-y-4">
              {r.instructions.map((step, i) => (
                <li key={i} className="flex gap-4 surface-card rounded-xl p-5">
                  <span className="grid place-items-center h-8 w-8 rounded-md bg-electric/15 text-electric font-display font-bold text-sm shrink-0">{i + 1}</span>
          ...
          <Section id="storage" eyebrow={t("recipe.storageSwaps")} title={t("recipe.makeItYours")}>
            <div className="grid md:grid-cols-2 gap-5">
              <div className="surface-card rounded-xl p-5">
                <p className="text-xs uppercase tracking-wider text-electric font-semibold">{t("recipe.storage")}</p>
                <ul className="mt-2 space-y-2">
                  {buildStorageBullets(r.storage).map((s) => <Bullet key={s}>{s}</Bullet>)}
                </ul>
              </div>
              <div className="surface-card rounded-xl p-5">
                <p className="text-xs uppercase tracking-wider text-electric font-semibold">{t("recipe.swaps")}</p>
                <ul className="mt-2 space-y-2">
          ...
          <section className="surface-card rounded-2xl p-8 md:p-10 relative overflow-hidden">
            ...
            <div className="relative grid md:grid-cols-[1fr_auto] gap-6 items-center">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-electric font-semibold">{t("recipe.cross.eyebrow")}</p>
                <h3 className="mt-2 font-display text-2xl md:text-3xl font-bold">{t("recipe.cross.title")}</h3>
                <p className="mt-2 text-muted-foreground">{t("recipe.cross.desc")}</p>
              </div>
              <Link to="/programs" className="inline-flex items-center justify-center rounded-md bg-electric px-5 py-3 text-sm font-bold text-onyx-50 hover:bg-electric-glow transition-colors">{t("recipe.cross.cta")}</Link>
            </div>
          </section>
        </div>
      </div>
...
function Bullet({ children }: { children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-3 text-sm text-foreground/85">
      <svg className="h-4 w-4 mt-0.5 text-electric shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
      <span>{children}</span>
    </li>
  );
}
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`min-w-0 rounded-md border px-2 py-2 text-center ${accent ? "bg-electric/10 border-electric/30" : "bg-onyx-100 border-border"}`}>
      <p className="text-[9px] uppercase tracking-wide text-muted-foreground leading-tight">{label}</p>
      <p className={`mt-0.5 font-display font-bold text-sm ${accent ? "text-electric" : ""}`}>{value}</p>
    </div>
  );
}
...
function AllergenPanel({ recipe }: { recipe: Recipe }) {
  ...
  return (
    <div className={`mt-6 rounded-xl border p-4 ${hasNuts ? "bg-red-500/10 border-red-500/40" : "bg-amber-500/5 border-amber-500/25"}`}>
      <div className="flex items-start gap-3">
        <svg className={`h-5 w-5 mt-0.5 shrink-0 ${hasNuts ? "text-red-400" : "text-amber-400"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-foreground/90 font-semibold">{allergenSummary(allergens)}</p>
          {allergens.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {allergens.map((a) => {
                const nut = a === "Peanuts" || a === "Tree Nuts";
                return (
                  <span key={a} className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded border ${nut ? "bg-red-500/20 border-red-500/50 text-red-200" : "bg-onyx-100 border-border text-muted-foreground"}`}>
                    {a}
                  </span>
                );
              })}
