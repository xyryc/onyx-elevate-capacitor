...
import { coachBySlug, coaches, type Coach } from "@/data/coaches";
...
export const Route = createFileRoute("/coaches/$slug")({
  ...
  head: ({ loaderData }) => {
    const c = loaderData?.coach;
    return {
      meta: c
        ? [
            { title: `${c.name} - ${c.role} | Onyx Elevate` },
            { name: "description", content: c.bio.slice(0, 155) },
            { property: "og:title", content: `${c.name} - ${c.role}` },
            { property: "og:description", content: c.bio.slice(0, 155) },
            { property: "og:image", content: c.img },
          ]
        : [{ title: "Coach - Onyx Elevate" }],
    };
  },
  notFoundComponent: () => (
    <div className="container-onyx py-32 text-center">
      <h1 className="font-display text-3xl font-bold">Coach not found</h1>
