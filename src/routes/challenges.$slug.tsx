...
function NotFoundView() {
  ...
  return (
    <div className="container-onyx py-24 text-center">
      ...
      <Link to="/challenges" className="mt-6 inline-block text-electric font-semibold">
        {t("challenges.d.backToChallenges")}
      </Link>
    </div>
  );
}
...
function ChallengeDetail() {
  ...
  useEffect(() => {
    let a = true;
    supabase.rpc("get_challenge_participant_count", { slug: ch.slug }).then(({ data }) => {
      if (a) setParticipantCount((data as number | null) ?? 0);
    });
    ...
    if (user) {
      ...
      supabase
        ...
        .maybeSingle()
        .then(({ data }) => {
          if (a && data) setReward(data);
        });
      if (storageKey) {
        try {
          setLastLogged(localStorage.getItem(storageKey));
        } catch {
          /* ignore */
        }
      }
            ...
            <div className="max-w-3xl">
              <div className="flex flex-wrap gap-2 text-[10px] uppercase tracking-wider">
                <span className="px-2 py-1 rounded-md bg-electric text-onyx-50 font-bold">
                  {t("challenges.d.daysBadge", { count: ch.durationDays })}
                </span>
          ...
          <article className="max-w-2xl">
            ...
            {levels && (
              <section className="mt-14">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.25em] text-electric font-bold">
                      {t("challenges.d.dayByDay")}
                    </p>
                    <h2 className="mt-2 font-display text-2xl md:text-3xl font-bold">
                      {t("challenges.d.yourPlan")}
                      <span className="text-electric">.</span>
                    </h2>
                ...
                {currentLevel && (
                  <>
                    <p className="mt-4 text-sm text-foreground/75 italic border-l-2 border-electric/40 pl-3">
                          ...
                          {weeks.map((wk) => (
                            <div key={wk.week}>
                              <div className="flex items-baseline gap-3 mb-4">
                                <span className="font-display text-3xl font-bold text-electric/80 leading-none">
                                  W{wk.week}
                                </span>
                                <span className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
                                  {t("challenges.d.weekLabel", { n: wk.week })}
                                </span>
                                  ...
                                  return (
                                    <div key={d.day}>
                                      ...
                                      >
                                        ...
                                        <span
                                          className={`flex-1 min-w-0 text-sm truncate ${d.rest ? "text-muted-foreground italic" : "text-foreground/90"}`}
                                        >
            ...
            {!levels && (
              <section className="mt-14 border-l-2 border-electric pl-5">
                <p className="text-[10px] uppercase tracking-[0.25em] text-electric font-bold">
                  {t("challenges.d.howThisWorksEyebrow")}
                </p>
            ...
            <section className="mt-14 grid md:grid-cols-2 gap-10">
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] text-electric font-bold">
                  {t("challenges.d.rulesEyebrow")}
                </p>
                <h3 className="mt-2 font-display text-xl font-bold">
                  {t("challenges.d.playFair")}
                  <span className="text-electric">.</span>
                </h3>
                <ul className="mt-4 space-y-3">
                  {ch.rules.map((r) => (
                    <li key={r} className="flex gap-3 text-sm text-foreground/85 leading-relaxed">
                      <svg
                        className="h-4 w-4 text-electric shrink-0 mt-1"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2.5"
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] text-destructive font-bold">
                  {t("challenges.d.mistakesEyebrow")}
                </p>
                <h3 className="mt-2 font-display text-xl font-bold">
                  {t("challenges.d.avoidThese")}
            ...
            <section className="mt-14 relative overflow-hidden rounded-2xl bg-gradient-to-br from-electric/20 via-electric/5 to-transparent border border-electric/30 p-6 md:p-8">
              ...
              <div className="relative flex items-start gap-5">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-electric font-bold">
                    {t("challenges.d.yourRewardEyebrow")}
                  </p>
                  <p className="mt-2 font-display text-3xl md:text-4xl font-bold leading-[1.1]">
                    <span className="text-electric">{t("challenges.d.rewardHeadline1")}</span>{" "}
                    {t("challenges.d.rewardHeadline2")}
                  </p>
          ...
          <aside>
            <div className="sticky top-24 surface-card rounded-2xl p-6">
              <p className="text-[10px] uppercase tracking-[0.2em] text-electric font-semibold">
                {t("challenges.d.yourChallenge")}
              </p>
              <p className="mt-2 text-xs text-muted-foreground">
                {t("challenges.d.athletesJoined", { count: participantCount.toLocaleString() })}
              </p>

              {!user ? (
                <Link
                  to="/auth"
                  className="mt-5 block w-full rounded-md bg-electric px-4 py-3 text-center text-sm font-bold text-onyx-50 hover:bg-electric-glow"
                >
                  ...
                  <div>
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">
                        {t("challenges.d.dayOf", { current: progress, total: ch.durationDays })}
                      </span>
                    ...
                    reward ? (
                      <div className="rounded-md border border-electric/50 bg-electric/10 p-4 text-center">
                        <p className="text-[10px] uppercase tracking-[0.2em] text-electric font-bold">
                          {t("challenges.d.rewardCodeLabel", { percent: reward.discount_percent })}
                        </p>
                        ...
                        <div className="mt-3 flex gap-2">
                          <button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                toast.success(t("challenges.d.codeCopied"));
                              } catch {
                                toast.error(t("challenges.d.copyFailed"));
                              }
                            }}
                          ...
                          <Link
                            ...
                            onClick={() => {
                              try {
                                sessionStorage.setItem("onyx:discountCode", reward.code);
                              } catch {
                                /* ignore */
                              }
                            }}
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            toast.success(
                              t("challenges.d.codeUnlocked", { percent: r.discount_percent }),
                            );
                          } catch (e) {
                            toast.error(
                              e instanceof Error ? e.message : t("challenges.d.couldNotClaim"),
                            );
                          } finally {
                            setBusy(false);
                          }
                      ...
                      >
                        {discountPercent >= 100
                          ? t("challenges.d.claimFreeProgram")
                          : t("challenges.d.claimPercent", { percent: discountPercent })}
                      </button>
                    )
                  ) : (
                    <button
      ...
      <div className="rounded-lg border border-border bg-onyx-100/60 divide-y divide-border/60">
        {rows.map((r: any, i: number) => (
          <div key={r.user_id} className="flex items-center gap-3 p-3">
            <div
              className={`w-7 text-center font-display font-bold text-sm ${i < 3 ? "text-electric" : "text-muted-foreground"}`}
            >
              {i + 1}
            </div>
            {r.avatar_url ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold truncate">
                {r.display_name || t("challenges.d.onyxAthlete")}
              </div>
              <div className="text-xs text-muted-foreground">
                {r.completed_at
                  ? t("challenges.d.finished")
                  : t("challenges.d.daysCount", { progress: r.progress, total: totalDays })}
              </div>
            </div>
            <div className="w-24 h-1.5 rounded-full bg-onyx-200 overflow-hidden">
              <div
                className="h-full bg-electric"
                style={{ width: `${Math.min(100, (r.progress / totalDays) * 100)}%` }}
              />
            </div>
          </div>
        ))}
