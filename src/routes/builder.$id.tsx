...
function BuilderEditor() {
    ...
    if (program) {
      ...
      setWeeks(
        program.weeks.length
          ? program.weeks
          : [{ name: "Week 1", days: [{ name: "Day 1", exercises: [] }] }],
      );
    }
  }, [program]);
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      router.navigate({
        to: "/my-library",
        hash: "custom-programs",
        search: { highlight: id } as any,
      });
    },
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      updateFn({ data: { id, name: latestRef.current.name, weeks: latestRef.current.weeks } })
        ...
        .catch(() => {
          /* silent — user can still hit Save */
        });
    }, 800);
  ...
  if (isLoading) {
    return (
      <div className="container-onyx py-20 text-center">
  ...
  if (!program) {
    return (
      <div className="container-onyx py-16 text-center">
        ...
        <Link to="/builder" className="mt-4 inline-flex text-sm text-electric">
  ...
  const addDay = (wi: number) =>
    setWeeks((ws) =>
      ws.map((w, i) =>
        i === wi
          ? { ...w, days: [...w.days, { name: `Day ${w.days.length + 1}`, exercises: [] }] }
          : w,
      ),
    );
  ...
  const removeDay = (wi: number, di: number) =>
    setWeeks((ws) =>
      ws.map((w, i) => (i === wi ? { ...w, days: w.days.filter((_, idx) => idx !== di) } : w)),
    );
      ...
      ws.map((w, i) =>
        i === wi
          ? {
              ...w,
              days: w.days.map((d, j) =>
                j === di ? { ...d, exercises: [...d.exercises, ex] } : d,
              ),
            }
          : w,
      ),
          ...
          ? {
              ...
              days: w.days.map((d, j) =>
                j === di
                  ? {
                      ...d,
                      exercises: d.exercises.map((e, k) => (k === ei ? { ...e, ...patch } : e)),
                    }
                  : d,
              ),
            }
            ...
            <section key={wi}>
              <div className="mb-4 flex items-center justify-between gap-3 border-b border-white/10 pb-2">
                ...
                {weeks.length > 1 && (
                  <button
                    onClick={() => {
                      if (confirm(t("builder.confirmRemoveWeek"))) removeWeek(wi);
                    }}
              ...
              <div className="space-y-4">
                {w.days.map((d, di) => (
                  <div
                    key={di}
                    className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 sm:p-5 shadow-[0_1px_0_0_rgba(255,255,255,0.04)_inset]"
                  >
                    <div className="mb-3 flex flex-wrap items-center justify-between gap-2 sm:gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        {(() => {
                          ...
                          const disabled =
                            done || markMut.isPending || d.exercises.length === 0 || blockedToday;
                          return (
                            <button
                              onClick={() =>
                                markMut.mutate({ dayKey, title: `${w.name} · ${d.name}` })
                              }
                              disabled={disabled}
                              ...
                              title={
                                d.exercises.length === 0
                                  ? t("builder.addExercisesFirst")
                                  : done
                                    ? t("builder.completed")
                                    : blockedToday
                                      ? "Kom tilbake i morgen for å logge neste dag"
                        ...
                        {w.days.length > 1 && (
                          <button
                            onClick={() => {
                              if (confirm(t("builder.confirmRemoveDay"))) removeDay(wi, di);
                            }}
                        ...
                        {d.exercises.map((ex, ei) => (
                          <div key={ei} className="rounded-lg bg-white/[0.05] p-3">
                            <div className="mb-2 flex items-start gap-2">
                              ...
                              >
                                <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full border border-electric/30 bg-electric/15 group-hover:bg-electric/30">
                                  <svg
                                    className="h-3 w-3 translate-x-[1px] text-electric"
                                    viewBox="0 0 24 24"
                                    fill="currentColor"
                                  >
                                ...
                                <span className="min-w-0 flex-1 break-words font-semibold text-sm sm:text-base underline-offset-4 group-hover:underline">
                                  {ex.exerciseName}
                                </span>
                              </Link>
                              <button
                                onClick={() => {
                                  if (confirm(t("builder.confirmRemoveExercise")))
                            ...
                            <div className="grid grid-cols-2 gap-2 sm:grid-cols-6">
                              {(["sets", "reps", "rpe", "rest", "tempo", "notes"] as const).map(
                                (f) => {
                                  const isRpe = f === "rpe";
                                  return (
                                    <label key={f} className="flex flex-col gap-1">
                                      <span
                                        className={`text-[10px] uppercase tracking-wider ${isRpe ? "text-electric" : "text-muted-foreground"}`}
                                        title={isRpe ? t("builder.rpeTooltip") : undefined}
                                      >
                                        {t(`builder.field.${f}`)}
                                        {isRpe && (
                                          <span className="ml-1 inline-block text-[9px] opacity-70">
                                            ⓘ
                                          </span>
                                        )}
                                      </span>
                                      <input
                                        value={ex[f] ?? ""}
                                        onChange={(e) =>
                                          updateExercise(wi, di, ei, { [f]: e.target.value } as any)
                                        }
                                        placeholder={
                                          f === "sets"
                                            ? "3"
...
function SavingOverlay() {
  const t = useT();
  const steps = [t("builder.savingStep1"), t("builder.savingStep2"), t("builder.savingStep3")];
...
function ExercisePicker({
  ...
  onClose: () => void;
  onPickMany: (exs: CustomExercise[]) => void;
}) {
  const t = useT();
                ...
                <button
                  ...
                  className={`group relative flex w-full flex-col overflow-hidden rounded-lg border text-left transition hover:-translate-y-0.5 ${
                    isSelected
                      ...
                      : "border-border bg-onyx-100 hover:border-electric/60"
                  }`}
                >
                  <div className="relative aspect-video w-full overflow-hidden bg-onyx-200">
                    ...
                    <div
                      className={`pointer-events-none absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-full border transition ${
                        isSelected
                          ? "border-emerald-400 bg-emerald-500 text-onyx-50"
