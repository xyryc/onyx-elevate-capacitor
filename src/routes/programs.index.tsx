...
const VALID_CATS = new Set<string>([
  "Strength", "Hypertrophy", "Bodybuilding", "Fat Loss", "Powerlifting",
  "Endurance", "Home Training", "Women", "Beginner", "Hyrox",
]);

export const Route = createFileRoute("/programs/")({
  validateSearch: (search: Record<string, unknown>) => ({
    cat: typeof search.cat === "string" && VALID_CATS.has(search.cat) ? (search.cat as string) : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Training Programs - Onyx Elevate" },
      { name: "description", content: "Free 1-week samplers plus full premium training programs from Onyx: bodybuilding, powerlifting, Hyrox, running, fat loss and more. Every exercise is wired to a video demo. R$ 29,99 each." },
      { property: "og:title", content: "Training Programs - Onyx Elevate" },
      { property: "og:description", content: "Try one free week, then unlock the full program for R$ 29,99." },
    ],
  }),
...
function ProgramsPage() {
  ...
  const byCategory: { key: string; eyebrow: string; title: string; subtitle: string; items: Program[] }[] = [];
  for (const c of programCategories) {
    const items = premiumPrograms.filter((p) => p.category === c);
    if (items.length === 0) continue;
    byCategory.push({ key: c, eyebrow: c === "Women" ? "Glute" : c, title: c === "Women" ? "Glute Specialization" : `${c} Programs`, subtitle: categoryBlurb(c), items });
  }
    ...
    <div>
      ...
      <section className="border-b border-border/60 bg-[radial-gradient(ellipse_at_top,_oklch(0.7_0.22_240/0.18),_transparent_60%)]">
        <div className="container-onyx py-10 md:py-14">
          <p className="text-xs uppercase tracking-[0.2em] text-electric font-semibold">{t("programs.eyebrow")}</p>
          <h1 className="mt-2 font-display text-3xl md:text-5xl font-bold leading-[1.05] max-w-3xl">
            {t("programs.heroTitle1")} <span className="text-gradient-electric">{t("programs.heroTitle2")}</span>
          </h1>
          {!hasMembership && (
            <p className="mt-3 text-base md:text-lg text-muted-foreground max-w-2xl">
              {t("programs.heroTaglinePre")} <span className="text-electric font-semibold">{programPrice}</span> {t("programs.heroTaglinePost")}
            </p>
          )}
      ...
      {filtered.length === 0 && (
        <div className="container-onyx py-16 text-center text-muted-foreground">
          No programs match this filter yet. <Link to="/programs" search={{ cat: undefined }} className="text-electric font-semibold">Clear filter</Link>
        </div>
      )}
        ...
        <section id="all-access" className="container-onyx pt-8 scroll-mt-24">
          <div className="flex items-center justify-between gap-4 rounded-2xl border border-electric/30 bg-gradient-to-r from-electric/10 via-onyx-100/60 to-onyx-100/60 px-5 py-4 md:px-6 md:py-5 shadow-electric/10">
            <div className="min-w-0">
              <p className="text-[11px] uppercase tracking-[0.2em] font-semibold text-electric">{t("programs.members.eyebrow")}</p>
              <p className="mt-1 text-sm md:text-base text-foreground/90 line-clamp-2">{t("programs.members.subtitle")}</p>
            </div>
            <MembershipModal
              trigger={
                <button type="button" className="shrink-0 rounded-md bg-electric px-4 py-2.5 text-sm font-semibold text-onyx-50 hover:bg-electric-glow hover:shadow-electric transition-all">
      ...
      {byCategory.map((row) => (
        <Row key={row.key} eyebrow={row.eyebrow} title={row.title} subtitle={row.subtitle} items={row.items} layout="grid" />
      ))}
  ...
  return (
    <section className="container-onyx mt-10 md:mt-14">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[10px] uppercase tracking-[0.25em] text-electric font-semibold">{t("programs.quick.divider")}</span>
        <span className="h-px flex-1 bg-border" />
      </div>
      <div className="flex items-end justify-between gap-4 mb-3">
        <div className="min-w-0">
          <h2 className="font-display text-xl md:text-2xl font-bold truncate">{t("programs.quick.title")}</h2>
          <p className="mt-0.5 text-sm text-muted-foreground line-clamp-2">{t("programs.quick.subtitle")}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs text-muted-foreground">{quickWorkouts.length} {t("programs.quick.count")}</span>
      ...
      >
        <div className="flex gap-3 items-stretch min-w-min">
          {quickWorkouts.map((w) => (
            <div key={w.slug} className="w-[75vw] max-w-[280px] sm:w-[240px] shrink-0 snap-start flex">
      ...
      >
        ...
        <div className="p-2.5 flex flex-col flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-1 text-[10px] uppercase tracking-wider text-muted-foreground">
            <span className="px-1 py-[1px] rounded bg-onyx-200 border border-border">{tagLabel}</span>
            <span className="px-1 py-[1px] rounded bg-onyx-200 border border-border">{w.minutes} {t("programs.quick.min")}</span>
          </div>
        </div>
      </article>
    </QuickWorkoutDialog>
...
function categoryBlurb(c: ProgramCategory): string {
  switch (c) {
    case "Bodybuilding": return "Classic splits, dual frequency, real growth.";
    case "Powerlifting": return "Squat, bench, deadlift - and the meet day plan.";
    case "Hypertrophy": return "Volume science applied to the muscles you actually want.";
    case "Fat Loss": return "Lift heavy, condition smart, keep the muscle.";
    case "Endurance": return "From your first 5K all the way to a marathon.";
    case "Hyrox": return "Race-prep work for every Hyrox station.";
    case "Home Training": return "Two dumbbells and a band - no excuses.";
    case "Women": return "Glute-focused programming built for growth, shape and strength.";
    case "Beginner": return "Master the lifts. Build the body that earns them.";
    case "Strength": return "Pure barbell strength, anchored on the big three.";
    case "Strongman": return "Sled, yoke, carries and stones - built for real-world power.";
  }
}

function Row({ eyebrow, title, subtitle, items, accent, layout = "grid" }: { eyebrow: string; title: string; subtitle: string; items: Program[]; accent?: "emerald"; layout?: "grid" | "scroll" }) {
  return (
    <section className="container-onyx mt-8 md:mt-10">
      <div className="flex items-end justify-between gap-4 mb-3">
        <div className="min-w-0">
          <p className={`text-[10px] uppercase tracking-[0.2em] font-semibold ${accent === "emerald" ? "text-emerald-300" : "text-electric"}`}>{eyebrow}</p>
        ...
        <span className="text-xs text-muted-foreground shrink-0">{items.length} program{items.length !== 1 && "s"}</span>
      </div>
      {layout === "scroll" ? (
        <div className="-mx-4 px-4 overflow-x-auto pb-3 [scrollbar-width:thin]">
          <div className="flex gap-4 min-w-min items-stretch">
            {items.map((p) => <CompactCard key={p.slug} p={p} />)}
          </div>
        </div>
      ) : (
