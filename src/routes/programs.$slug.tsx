...
function ProgramNotFound() {
  ...
  return (
    <div className="container-onyx py-24 text-center">
      ...
      <Link to="/programs" className="mt-6 inline-block text-electric font-semibold">
        ← {t("Back to programs")}
      </Link>
    </div>
  );
...
function ProgramDetail() {
  ...
  const Prose = ({ children, className }: { children: string; className?: string }) =>
    isBeginner ? (
      <GlossaryText className={className}>{children}</GlossaryText>
    ) : (
    ...
    <div>
          ...
          <div className="p-6 md:p-10">
            <div className="max-w-3xl">
              <div className="flex flex-wrap gap-2 text-[10px] uppercase tracking-wider">
                ...
                <span className="px-2 py-1 rounded-md bg-onyx-100 border border-border text-muted-foreground">
                  {p.duration}
                </span>
                <span className="px-2 py-1 rounded-md bg-onyx-100 border border-border text-muted-foreground">
              ...
              <h1 className="mt-4 font-display text-3xl md:text-5xl lg:text-6xl font-bold leading-[1.05]">
                {p.title}
              </h1>
              <p className="mt-3 md:mt-4 text-base md:text-lg text-muted-foreground max-w-2xl">
                <Prose>{p.tagline}</Prose>
              </p>
              ...
              <p className="mt-2 text-[11px] text-muted-foreground">
                Adds the plan to <span className="text-electric font-semibold">My Library</span> so
                you can check off each day as you train.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container-onyx grid lg:grid-cols-[260px_1fr] gap-10 pb-24">
        ...
        <div className="min-w-0 space-y-14">
          ...
          <Section
            id="warmup"
            eyebrow="Pre-session protocol"
            title="Warm-up · do this before every workout"
          >
            <MobileCollapse
            ...
            >
              ...
              <div className="mt-6 grid lg:grid-cols-3 gap-4">
                <WarmupColumn
                  title="1 · General prep"
                  subtitle="Raise temperature"
                  steps={warmup.generalPrep}
                />
                <WarmupColumn
                  title="2 · Specific prep"
                  subtitle="Rehearse the lift"
                  steps={warmup.specificPrep}
                />
                <WarmupColumn
                  title="3 · Activation"
                  subtitle="Fire the prime movers"
                  steps={warmup.activation}
                />
              </div>
          ...
          <Section
            id="nutrition"
            eyebrow={t("program.nutrition")}
            title={`${t("program.nutrition")} · ${p.category}`}
          >
            <p className="text-foreground/85 leading-relaxed">
              <Prose>{p.nutrition}</Prose>
            </p>
          </Section>

          <Section
            id="supplementation"
            eyebrow={t("program.supplementation")}
            title={t("program.supplementation")}
          >
            <p className="text-foreground/85 leading-relaxed">
              <Prose>{p.supplementation}</Prose>
            </p>
          </Section>

          <Section id="recovery" eyebrow={t("program.recovery")} title={t("program.recovery")}>
            <p className="text-foreground/85 leading-relaxed">
              <Prose>{p.recovery}</Prose>
            </p>
          </Section>

          <Section
            id="training"
            eyebrow={t("program.trainingOverview")}
            title={t("program.trainingOverview")}
          >
            <p className="text-foreground/85 leading-relaxed">
              <Prose>{p.trainingOverview}</Prose>
            </p>
          </Section>

          <Section
            id="progression"
            eyebrow={`${p.duration} · ${t("program.progression")}`}
            title={t("program.progression")}
          >
            <p className="text-foreground/85 leading-relaxed">
              <Prose>{p.progression}</Prose>
            </p>
          ...
          <Section
            id="schedule"
            eyebrow={t("program.weeklySchedule")}
            title={t("program.weeklySchedule")}
          >
            <details className="group surface-card rounded-2xl border border-electric/20 overflow-hidden [&_summary::-webkit-details-marker]:hidden">
              <summary className="flex items-center justify-between cursor-pointer gap-4 p-5 md:p-6 hover:bg-onyx-100/30 transition-colors">
                <div className="min-w-0">
                  ...
                  <h3 className="mt-1.5 font-display text-lg md:text-xl font-bold">
                    {t("Your 7-day training split")}
                  </h3>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {t("Tap to see which day trains what")}
                  </p>
                </div>
                <svg
                  className="h-6 w-6 text-electric shrink-0 transition-transform group-open:rotate-180"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
          ...
          {p.category === "Endurance" && (
            <section className="surface-card rounded-2xl p-6 md:p-8 border border-electric/20">
              ...
              <p className="mt-2 text-sm text-muted-foreground max-w-2xl">
                Every endurance session is built around heart-rate zones so you train the right
                system on the right day. Use a watch, chest strap, or the talk-test to stay in the
                target band.
              </p>
              ...
              <p className="mt-5 text-xs text-muted-foreground">
                <span className="text-electric font-semibold">Tip:</span> Estimate your max heart
                rate with 220 minus your age, then match the percentages above. Adjust by feel - the
                talk-test never lies.
              </p>
            </section>
          )}
              ...
              <details className="group surface-card rounded-2xl border border-electric/20 overflow-hidden [&_summary::-webkit-details-marker]:hidden">
                <summary className="flex items-center justify-between cursor-pointer gap-4 p-6 hover:bg-onyx-100/30 transition-colors">
                  <div className="min-w-0">
                    ...
                    <h3 className="mt-1.5 font-display text-xl md:text-2xl font-bold">
                ...
                <div className="px-6 pb-6 pt-1 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {PREP_TIPS.map((tip) => (
                    <div
                      key={tip.title}
                      className="rounded-xl bg-onyx-100/50 p-4 border border-border"
                    >
          ...
          <Section id="faq" eyebrow={t("program.faq")} title={t("program.faq")}>
            <div className="space-y-3">
              {p.faqs.map((f) => (
                <details
                  key={f.question}
                  className="group surface-card rounded-xl p-5 [&_summary::-webkit-details-marker]:hidden"
                >
                  <summary className="flex items-center justify-between cursor-pointer gap-4">
                    ...
                    <svg
                      className="h-5 w-5 text-electric shrink-0 transition-transform group-open:rotate-180"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
    ...
    return (
      <div className="surface-card rounded-xl p-5 border border-electric/40">
        <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-electric">
          ✓ {bundle ? t("purchase.allAccess") : t("purchase.unlocked")}
        </p>
  ...
  return (
    <div className={`surface-card rounded-xl p-5 ${isFree ? "" : "glow-ring"}`}>
      <div className="flex items-baseline justify-between gap-3">
        ...
        <span
          className={`text-xs px-2 py-0.5 rounded-md font-bold uppercase tracking-wider ${isFree ? "bg-emerald-400/15 text-emerald-300 border border-emerald-400/30" : "bg-electric/15 text-electric border border-electric/30"}`}
        >
      ...
      {!isFree && (
        <MembershipModal
          trigger={<ShimmerButton className="mt-5 w-full">{t("Become a member")}</ShimmerButton>}
        />
      )}
    </div>
  );
...
const PREP_TIPS = [
  {
    title: "Pack your bag the night before",
    body: "Shaker, belt, straps, sleeves, headphones. Zero friction in the morning = zero excuses.",
  },
  {
    title: "Eat 60-90 min before lifting",
    body: "Slow carb + lean protein (oats + whey, rice + chicken). Fuels heavy work without sitting heavy in the stomach.",
  },
  {
    title: "Hydrate early",
    body: "500 ml water + a pinch of salt 30 min before training. Cramping and brain-fog usually = dehydration, not weakness.",
  },
  {
    title: "Warm-up is non-negotiable",
    body: "Run the full 3-stage protocol above. Skipping it is the fastest way to leave 10% on the platform.",
  },
  {
    title: "Log every set",
    body: "Weight, reps, RPE. If you don't track it, you can't progress it. Use the Onyx app or a notebook, just log.",
  },
  {
    title: "Cap rest with a timer",
    body: "Set the rest column as a timer on your watch. Drifting past rest kills intensity and stretches sessions 2x.",
  },
  {
    title: "Protein within 90 min post-session",
    body: "0.4 g/kg bodyweight. Whey + banana works, real meal works better. Recovery starts here.",
  },
  {
    title: "Plan deload weeks",
    body: "Every 4-6 weeks drop volume 40%. Strength is built in recovery, not in extra sets.",
  },
];
