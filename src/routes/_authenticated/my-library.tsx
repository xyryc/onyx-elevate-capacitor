...
function MyLibrary() {
  ...
  const { data: customPrograms = [] } = useQuery({
    queryKey: ["custom-programs"],
    queryFn: () => listCustomPrograms(),
  });
  const { data: aiMealPlans = [] } = useQuery({
    queryKey: ["ai-meal-plans"],
    queryFn: () => listMyAiMealPlans(),
  });
  let stripeEnv: "sandbox" | "live" | null = null;
  try {
    stripeEnv = getStripeEnvironment();
  } catch {
    stripeEnv = null;
  }
  ...
  const ownedPlans = hasBundle
    ? nutritionPlans
    : nutritionPlans.filter((pl) =>
        purchases.some(
          (x) =>
            x.product_kind === "plan" &&
            (x.product_slug === pl.slug || x.product_slug === `plan:${pl.slug}`),
        ),
      );
  ...
  for (const p of savedMealPlans)
    if (!combinedPlansMap.has(p.slug)) combinedPlansMap.set(p.slug, p);
  const visibleOwnedPlans = Array.from(combinedPlansMap.values()).filter(
    (p) => !hiddenPlanSlugs.includes(p.slug),
  );
  ...
  const memberTierLabel =
    membership?.tier === "lifetime"
      ? t("library.memberLifetime")
      : membership?.tier === "yearly"
        ? t("library.memberYearly")
        : membership?.tier === "monthly"
          ? t("library.memberMonthly")
          : undefined;
      ...
      <div className="container-onyx pt-0 pb-8 lg:py-12 relative">
        ...
        <div className="mt-6 flex justify-center gap-3 pb-2">
          <QuickTile
            to="/challenges"
            icon={Trophy}
            title={t("library.challengesTitle")}
            subtitle={t("library.tileChallengesSub")}
            tone="purple"
            image={tileChallengesImg}
          />
          <QuickTile
            icon={Bookmark}
        ...
        <section className="mt-8 border-t border-border/40 pt-5">
          <div className="flex items-end justify-between gap-3">
            ...
            {activePrograms.length +
              visibleOwnedPrograms.filter(
                (p) => !activePrograms.some((ap) => ap.program.slug === p.slug),
              ).length >
              3 && (
              <button
                onClick={() => openSeeAll("programs")}
          ...
          {isLoading ? (
            ...
            <div className="mt-3 rounded-xl border border-border bg-onyx-100 p-8 text-center">
              ...
              <Link
                to="/programs"
                className="mt-4 inline-block rounded-md bg-electric px-5 py-2.5 text-sm font-bold text-onyx-50 hover:bg-electric-glow"
              >
                {t("library.browsePrograms")}
              </Link>
            </div>
          ) : (
            (() => {
              const restPrograms = visibleOwnedPrograms.filter(
                (p) => !activePrograms.some((ap) => ap.program.slug === p.slug),
              );
        ...
        <section className="mt-8 border-t border-border/40 pt-5">
          <div className="flex items-center justify-between gap-3">
            ...
            {customPrograms.length > 0 && (
              <button
                onClick={() => openSeeAll("custom")}
                className="text-xs text-electric font-semibold hover:underline whitespace-nowrap"
              >
                {t("library.browseAll")}
              </button>
            )}
          ...
          {customPrograms.length === 0 ? (
            <div className="mt-3 rounded-xl border border-border bg-onyx-100 p-6 text-sm text-muted-foreground">
              {t("library.noCustomPrograms")}{" "}
              <Link to="/builder" className="text-electric font-semibold">
                {t("library.buildYourOwn")}
              </Link>
            </div>
          ) : (
        ...
        <section className="mt-8 border-t border-border/40 pt-5">
          ...
          {myChallenges.length === 0 ? (
            <div className="mt-3 rounded-xl border border-border bg-onyx-100 p-6 text-sm text-muted-foreground">
              {t("library.noChallenges")}{" "}
              <Link to="/challenges" className="text-electric font-semibold">
                {t("library.pickChallenge")}
              </Link>
            </div>
          ) : (
        ...
        <SeeAllDialog
          open={seeAll === "programs"}
          onOpenChange={(v) => !v && closeSeeAll()}
          title={t("library.myPrograms")}
        >
          {activePrograms.map(({ program, progress, pct, totalDays }) => (
            <ActiveProgramCard
              key={program.slug}
          ...
          {visibleOwnedPrograms
            .filter((p) => !activePrograms.some((ap) => ap.program.slug === p.slug))
            .map((p) => (
              <OwnedProgramCard key={p.slug} program={p} hiddenSlugs={hiddenProgramSlugs} />
            ))}
        </SeeAllDialog>
        <SeeAllDialog
          open={seeAll === "custom"}
          onOpenChange={(v) => !v && closeSeeAll()}
          title={t("library.myCustomPrograms")}
        >
          {customPrograms.map((cp) => (
        ...
        <SeeAllDialog
          open={seeAll === "challenges"}
          onOpenChange={(v) => !v && closeSeeAll()}
          title={t("library.myChallenges")}
        >
          ...
          <section className="mt-8 border-t border-border/40 pt-5">
            ...
            <CardRow count={visibleOwnedPlans.length + aiMealPlans.length}>
              {visibleOwnedPlans.map((p) => (
                <MealPlanCard
                  key={p.slug}
                  plan={findNutritionPlan(p.slug)!}
                  hiddenSlugs={hiddenPlanSlugs}
                />
              ))}
              {aiMealPlans.map((p) => (
        ...
        {(() => {
          ...
          return (
            <section
              id="saved-library-section"
              className="mt-8 border-t border-border/40 pt-5 scroll-mt-24"
            >
  ...
  return (
    <div className="relative group rounded-xl border border-border bg-onyx-100 hover:border-electric/60 transition-all overflow-hidden">
      ...
      <Link to="/training/$slug" params={{ slug: program.slug }} className="block">
        ...
        <div className="p-4">
          <div className="text-[10px] uppercase tracking-widest text-electric font-bold">
            {program.category} · {t("library.inProgress")}
          </div>
          <div className="mt-1 font-display font-bold group-hover:text-electric transition-colors pr-8 line-clamp-1">
            {program.title}
          </div>
          <div className="mt-3">
            <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">
                {t("library.daysProgress")
                  .replace("{{done}}", String(progress.completed_days.length))
          ...
          <div className="mt-3 flex items-center justify-between text-xs">
            <span className="text-muted-foreground">
              {t("library.lastActive").replace(
                "{{date}}",
                new Date(progress.last_active_at).toLocaleDateString(),
              )}
            </span>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("library.removeProgramDescriptionStart")} <strong>{program.title}</strong>{" "}
              {t("library.removeProgramDescriptionEnd")
                .replace("{{done}}", String(progress.completed_days.length))
                .replace("{{total}}", String(totalDays))}
            </AlertDialogDescription>
          </AlertDialogHeader>
...
function FavoriteItem({
  ...
  if (type === "program") {
    ...
    const img =
      typeof (p as any).image === "string" ? (p as any).image : ((p as any).image?.url ?? null);
    return (
      <Link
        to="/training/$slug"
        params={{ slug }}
        className="group rounded-xl border border-border bg-onyx-100 hover:border-electric/60 transition-all block overflow-hidden"
      >
        {img && (
          <div className="h-24 sm:h-28 w-full overflow-hidden bg-onyx-50">
            <img
        ...
        <div className="p-3">
          <div className="text-[10px] uppercase tracking-widest text-electric font-bold">
            {t("library.programLabel")} · {p.category}
          </div>
          <div className="mt-1 font-display font-bold text-sm group-hover:text-electric line-clamp-1">
            {p.title}
          </div>
          {totalDays > 0 && (
            <div className="mt-2">
  ...
  if (type === "challenge") {
    ...
    if (!c) return null;
    const img =
      typeof (c as any).image === "string" ? (c as any).image : ((c as any).image?.url ?? null);
    return (
      <Link
        to="/challenges/$slug"
        params={{ slug }}
      ...
      >
        ...
        <div className="p-3">
          <div className="text-[10px] uppercase tracking-widest text-electric font-bold">
            {t("library.challengeLabel")} ·{" "}
            {t("library.daysCount").replace("{{count}}", String(c.durationDays))}
          </div>
          <div className="mt-1 font-display font-bold text-sm group-hover:text-electric line-clamp-1">
            {c.title}
          </div>
  ...
  if (type === "recipe") {
    ...
    const img =
      typeof (r as any).image === "string"
        ? (r as any).image
        : ((r as any).image?.url ?? (r as any).heroImage ?? null);
    return (
      <Link
        to="/recipes/$slug"
        params={{ slug }}
  ...
  if (type === "meal-plan") {
    ...
    if (!pl) return null;
    const img =
      typeof (pl as any).image === "string" ? (pl as any).image : ((pl as any).image?.url ?? null);
    return (
      <Link
        to="/meal-plans/$slug"
        params={{ slug }}
...
function QuickLink({
  ...
  return (
    <Link
      to={to as any}
      className="group flex items-center gap-3 rounded-xl border border-border bg-onyx-100 p-4 hover:border-electric/60 transition-all"
    >
      <div className="w-10 h-10 rounded-lg bg-electric/15 border border-electric/30 flex items-center justify-center">
        <Icon className="w-5 h-5 text-electric" />
      </div>
      <div className="min-w-0">
        <div className="font-display font-bold text-sm group-hover:text-electric truncate">
          {title}
        </div>
  ...
  const inner = (
    <>
      ...
      <div className="relative h-full p-3 flex flex-col">
        <div
          className={`w-9 h-9 rounded-xl ${c.bg} border ${c.ring} flex items-center justify-center backdrop-blur-sm`}
        >
        ...
        <div className="mt-1 text-[11px] leading-snug text-white/75 line-clamp-2">{subtitle}</div>
      </div>
    </>
  );
  const cls =
    "group relative shrink-0 w-[168px] h-[132px] overflow-hidden rounded-2xl border border-border bg-onyx-100 hover:border-electric/60 transition-all text-left";
  if (onClick) {
    return (
...
function CustomProgramCard({
  ...
  const totalEx = cp.weeks.reduce(
    (a, w) => a + w.days.reduce((b, d) => b + d.exercises.length, 0),
    0,
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setTimeout(
        () =>
          el.classList.remove("ring-2", "ring-electric", "ring-offset-2", "ring-offset-onyx-50"),
        2400,
      );
    }, 200);
  ...
  return (
    <div
      ref={rootRef}
      id={`custom-program-${cp.id}`}
      className="relative group h-full flex flex-col overflow-hidden rounded-xl border border-border bg-onyx-100 hover:border-electric/60 transition-all"
    >
      <button
        type="button"
      ...
      >
        ...
        <div className="flex flex-1 flex-col p-4">
          <div className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">
            {t(
              cp.weeks.length === 1 ? "library.weekCountSingular" : "library.weekCountPlural",
            ).replace("{{count}}", String(cp.weeks.length))}
          </div>
          <div className="mt-1 font-display font-bold group-hover:text-electric transition-colors line-clamp-2">
            {cp.name}
          </div>
          <div className="mt-1 text-xs text-muted-foreground">
            {t("library.customMeta")
              .replace("{{weeks}}", String(cp.weeks.length))
              .replace("{{days}}", String(totalDays))}
          </div>
          <div className="mt-3">
            <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">
                {t("library.loggedProgress")
          ...
          <div className="mt-auto pt-4 text-xs font-semibold text-electric">
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("library.removeConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("library.deleteCustomDescriptionStart")} <strong>{cp.name}</strong>{" "}
              {t("library.deleteCustomDescriptionEnd")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={removing}>{t("library.keepIt")}</AlertDialogCancel>
  ...
  return (
    <div className="relative group rounded-xl border border-border bg-onyx-100 hover:border-electric/60 transition-all overflow-hidden">
      ...
      <Link to="/training/$slug" params={{ slug: program.slug }} className="block">
        ...
        <div className="p-3">
          <div className="text-[10px] uppercase tracking-widest text-electric font-bold">
            {program.category} · {t("library.unlocked")}
          </div>
          <div className="mt-2 font-display font-bold group-hover:text-electric transition-colors pr-8">
            {program.title}
          </div>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("library.removeConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("library.removeSimpleDescriptionStart")} <strong>{program.title}</strong>{" "}
              {t("library.removeSimpleDescriptionEnd")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={removing}>{t("library.keepIt")}</AlertDialogCancel>
...
function MealPlanCard({ plan, hiddenSlugs }: { plan: any; hiddenSlugs: string[] }) {
  ...
  return (
    <div className="relative group rounded-xl border border-border bg-onyx-100 hover:border-electric/60 transition-all overflow-hidden">
      ...
      <Link to="/meal-plans/$slug" params={{ slug: plan.slug }} hash="weeks" className="block">
        {plan.image ? (
          <div className="h-24 sm:h-28 w-full overflow-hidden bg-onyx-50">
            <img
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("library.removeConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("library.removeSimpleDescriptionStart")} <strong>{plan.title}</strong>{" "}
              {t("library.removeSimpleDescriptionEnd")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
    ...
    <div className="relative group rounded-xl border border-border bg-onyx-100 hover:border-electric/60 transition-all overflow-hidden">
      ...
      <Link to="/challenges/$slug" params={{ slug: participant.challenge_slug }} className="block">
        ...
        <div className="p-3">
          <div className="text-[10px] uppercase tracking-widest text-electric font-bold">
            {ch.category} ·{" "}
            {participant.completed_at ? t("library.complete") : t("library.activeStatus")}
          </div>
          <div className="mt-2 font-display font-bold group-hover:text-electric transition-colors pr-8">
            {ch.title}
          </div>
          <div className="mt-3">
            <div className="flex items-center justify-between text-xs">
              <span className="text-muted-foreground">
                {t("library.dayProgress")
                  .replace("{{done}}", String(participant.progress))
                  .replace("{{total}}", String(ch.durationDays))}
              </span>
              <span className="text-electric font-bold">{pct}%</span>
            </div>
            <Progress value={pct} className="mt-1.5" />
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("library.removeConfirmTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("library.removeSimpleDescriptionStart")} <strong>{ch.title}</strong>{" "}
              {t("library.removeSimpleDescriptionEnd")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {t("library.aiMealPlanMeta")
            .replace("{{days}}", String(plan.days.length))
