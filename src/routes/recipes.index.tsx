  ...
  head: () => ({
    meta: [
      { title: "Healthy Recipes - Onyx Elevate" },
      {
        name: "description",
        content:
          "Free high-protein recipes built for athletes: post-workout meals, breakfasts, smoothies, snacks, vegan and low-carb options. Macros and prep times included.",
      },
      { property: "og:title", content: "Healthy Recipes - Onyx Elevate" },
      {
        property: "og:description",
        content: "Athlete-built recipes with full macros, prep times and pro tips. All free.",
      },
    ],
  }),
...
function RecipesPage() {
  ...
  const grouped = useMemo(() => {
    ...
    return RECIPE_CATEGORIES.filter((c) => map.has(c)).map((c) => ({
      category: c,
      items: map.get(c)!,
    }));
  }, [filtered]);
        ...
        <div className="container-onyx relative pt-10 md:pt-16 pb-6 md:pb-8">
          <div className="max-w-2xl lg:max-w-3xl">
            ...
            <h1 className="mt-3 font-display text-4xl md:text-6xl font-bold leading-[1.05]">
              {t("recipes.heroTitle1")}{" "}
              <span className="text-gradient-electric">{t("recipes.heroTitle2")}</span>
            </h1>
        ...
        <div className="container-onyx relative mt-6 md:mt-8 pb-8 md:pb-16 space-y-8 md:space-y-10">
          ...
          {grouped.map(({ category: cat, items }) => {
            const avgP = Math.round(
              items.reduce((s, r) => s + r.macrosPerServing.protein, 0) / items.length,
            );
            const avgC = Math.round(
              items.reduce((s, r) => s + r.macrosPerServing.carbs, 0) / items.length,
            );
            const avgF = Math.round(
              items.reduce((s, r) => s + r.macrosPerServing.fat, 0) / items.length,
            );
              ...
              <div key={cat}>
                ...
                <div className="mt-3 -mx-4 px-4 flex items-stretch gap-3 overflow-x-auto snap-x snap-mandatory pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:mx-0 lg:px-0 lg:grid lg:grid-cols-4 lg:gap-4 lg:overflow-visible lg:pb-0">
                  {items.map((r) => (
                    <div
                      key={r.slug}
                      className="w-[58%] sm:w-[42%] shrink-0 snap-start lg:w-auto lg:shrink flex"
                    >
                      <RecipeCard recipe={r} />
                    </div>
                  ))}
    ...
    <RecipeDialog recipe={r}>
      ...
      >
        <div className="relative aspect-[4/3] overflow-hidden bg-onyx-200">
          ...
          <div className="absolute top-2 left-2 right-2 flex flex-wrap gap-1">
            {r.diets.slice(0, 2).map((d) => (
              <span
                key={d}
                className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-onyx-50/70 backdrop-blur border border-border/60 text-foreground/90"
              >
                {t(d)}
              </span>
            ))}
            {r.diets.length > 2 && (
              <span className="text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-onyx-50/70 backdrop-blur border border-border/60 text-foreground/90">
                +{r.diets.length - 2}
              </span>
            )}
          </div>
        ...
        <div className="p-3 flex flex-col flex-1 bg-onyx-50/95">
          <h3 className="font-display text-sm font-bold leading-snug group-hover:text-electric transition-colors line-clamp-2 min-h-[2.5rem]">
            {t(r.title)}
          </h3>
          ...
          <div className="mt-2 flex items-center gap-3 text-[10px] uppercase tracking-wider font-semibold text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              <Dot color="electric" />
              <span>
                {r.macrosPerServing.protein}g <span className="text-foreground">P</span>
              </span>
            </span>
            <span className="inline-flex items-center gap-1">
              <Dot color="amber" />
              <span>
                {r.macrosPerServing.carbs}g <span className="text-foreground">C</span>
              </span>
            </span>
            <span className="inline-flex items-center gap-1">
              <Dot color="yellow" />
              <span>
                {r.macrosPerServing.fat}g <span className="text-foreground">F</span>
              </span>
            </span>
          </div>
        </div>
...
function Dot({ color }: { color: "electric" | "amber" | "yellow" }) {
  const cls =
    color === "electric" ? "bg-electric" : color === "amber" ? "bg-orange-400" : "bg-yellow-300";
